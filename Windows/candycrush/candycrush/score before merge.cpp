...
void drawImage(int i, int j) {
	...
	glBegin(GL_QUADS);
	glTexCoord2f(0.0, 1.0); glVertex2f(i, j);
	glTexCoord2f(1.0, 1.0); glVertex2f(i + 1, j);
	glTexCoord2f(1.0, 0.0); glVertex2f(i + 1, j + 1);
	glTexCoord2f(0.0, 0.0); glVertex2f(i, j + 1);
	glEnd();
...
void paintBlack(int i, int j, int count, int type, int option) {
	...
	else if (option == COLUMNS)	{		// i = columns, j = rows
		if (j < 3) { 
			y = 0;
			yy = j + 3;
		//	printf("1 j = %d\n", j);		// ok
	...
	for (int k = x; k <= xx; k++)
	{
		for (int n = y; n <= yy; n++)
		{
			//printf("type %d, current type = %d\n",type,squares[k][n]);
			if (option == ROWS && k >= midR - 1 && k <= midR + 1)
			{
				if (checkForType(type, squares[k][n],1)) {
					squares[k][n] = 5;
					score += 2;
				}
				else {
...
void findMatches() {
	...
	for (int i = 0; i < ROWS; i++)
	{
		...
		for (int j = 1; j <= COLUMNS; j++)
		{
			...
			else if (count >= 3 && squares[i][j] != type)
			{
				int k = count;
				score += 10;
				while (k >= 1)
...
void highlightSquare(int i, int j)
{
	...
	glVertex2f(i, j + 1);
	glEnd();
	//glutSwapBuffers();    // kathe fora h displaygrid t xanakanei maura g auto dn fainontai
}
